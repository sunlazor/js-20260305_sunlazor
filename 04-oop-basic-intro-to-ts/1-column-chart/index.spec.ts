...
describe('oop-basic-intro-to-dom/column-chart', () => {
  ...
  it('should have ability to define total value', () => {
    ...
    const columnLink = columnChart.element?.querySelector('[data-element="header"]');

    expect(columnLink).toHaveTextContent(String(value));
  });

  it('should have ability to define "formatHeading" function', () => {
    const formatHeading = (data: number) => `USD ${data}`;
    const value = 100;
  ...
  it('should render data correctly', () => {
    ...
    const [firstBar, secondBar, thirdBar] = Array.from(chart!.children) as HTMLElement[];

    expect(getComputedStyle(firstBar).getPropertyValue('--value')).toEqual(columnProps[0].value);
    expect(getComputedStyle(secondBar).getPropertyValue('--value')).toEqual(columnProps[1].value);
    expect(getComputedStyle(thirdBar).getPropertyValue('--value')).toEqual(columnProps[2].value);

    expect(firstBar.dataset.tooltip).toEqual(columnProps[0].percent);
    expect(secondBar.dataset.tooltip).toEqual(columnProps[1].percent);
    expect(thirdBar.dataset.tooltip).toEqual(columnProps[2].percent);
  });
  ...
  it('should have ability to be updated by new "data" values (should re-render only body with charts columns)', () => {
    ...
    const [firstBar] = Array.from(chart!.children) as HTMLElement[];
...
function getColumnProps(data: number[]): Array<{ percent: string; value: string }> {
