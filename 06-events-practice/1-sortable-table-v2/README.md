...
```typescript
new SortableTable(headersConfig, { data, sorted })
```

* `headersConfig`: конфигурация колонок таблицы.
* `data`: массив строк таблицы.
* `sorted`: объект начальной сортировки вида `{ id, order }`.
...
* `destroy()`: удаляет компонент из DOM и очищает обработчики.

## Инструкция по запуску

**Запуск в режиме разработки:**
```bash
npm run dev:06:1-sortable-table-v2
```

**Запуск тестов:**
```bash
npm test 06-events-practice/1-sortable-table-v2
```
